use ghx_proc_gen::{
    generator::{builder::GeneratorBuilder, model::ModelCollection, rules::RulesBuilder, RngMode,
...
fn solve_grid(
    ...
    let (_info, grid_data) = generator.generate_grid().expect("build.rs: generation failed");
...
fn lava(rf: f32) -> Vec<u8> {
    solve_grid(|sockets, models| {
        ...
        models.create(SocketsCartesian2D::Simple {
            x_pos: s_run, x_neg: s_run, y_pos: s_edge, y_neg: s_edge,
        }).with_weight(5.0);
        // Corner
        models.create(SocketsCartesian2D::Simple {
            x_pos: s_run, x_neg: s_edge, y_pos: s_run, y_neg: s_edge,
        }).with_weight(0.1).with_all_rotations();
        // T-junction
        models.create(SocketsCartesian2D::Simple {
            x_pos: s_run, x_neg: s_run, y_pos: s_run, y_neg: s_edge,
        }).with_weight(0.5).with_all_rotations();
        // Cross
